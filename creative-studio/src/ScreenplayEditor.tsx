...
export default function ScreenplayEditor({ initialScript, title, onTitleChange }: { initialScript: string; title: string; onTitleChange: (title: string) => void }) {
  ...
  return <div className={`editor-layout ${utilityOpen ? 'utility-open' : 'utility-closed'} ${draggingFile ? 'dragging-file' : ''}`} onDragEnter={(event) => { if (event.dataTransfer.types.includes('Files')) { event.preventDefault(); setDraggingFile(true) } }} onDragOver={(event) => { if (event.dataTransfer.types.includes('Files')) event.preventDefault() }} onDragLeave={(event) => { if (draggingFile && !event.currentTarget.contains(event.relatedTarget as Node)) setDraggingFile(false) }} onDrop={(event) => { if (!event.dataTransfer.types.includes('Files')) return; event.preventDefault(); setDraggingFile(false); const file = event.dataTransfer.files[0]; if (file) openFile(file) }}>
    <aside className="scene-nav"><div className="aside-title"><span>SCENES</span><b>{scenes.length}</b></div><nav onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setSceneDropIndex(null) }}>{scenes.map(({ block }, index) => <button key={block.id} draggable onDragStart={(event) => { event.stopPropagation(); hideNativeDragPreview(event); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('application/x-mise-scene', String(index)); setDraggedScene(index); setSceneDropIndex(null) }} onDragEnter={(event) => event.stopPropagation()} onDragOver={(event) => { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; const bounds = event.currentTarget.getBoundingClientRect(); setSceneDropIndex(event.clientY < bounds.top + bounds.height / 2 ? index : index + 1) }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); if (draggedScene !== null && sceneDropIndex !== null) moveScene(draggedScene, sceneDropIndex); setDraggedScene(null); setSceneDropIndex(null) }} onDragEnd={() => { setDraggedScene(null); setSceneDropIndex(null) }} className={`${index === activeSceneIndex ? 'active' : ''} ${sceneDropIndex === index + 1 ? 'drop-after' : ''} ${sceneDropIndex === 0 && index === 0 ? 'drop-before' : ''}`} onClick={() => { setActiveId(block.id); fields.current.get(block.id)?.scrollIntoView({ behavior: 'smooth', block: 'center' }); focusBlock(block.id) }}><i>⠿</i><span>{String(index + 1).padStart(2, '0')}</span><p>{block.text || 'UNTITLED SCENE'}</p></button>)}</nav><button className="new-scene" onClick={() => insertAt(blocks.length, 'scene')}>＋ New scene</button><div className="nav-tip">Drag scenes to reorder them.</div></aside>
