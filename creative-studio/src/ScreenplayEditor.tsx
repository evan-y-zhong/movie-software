...
type Block = { id: string; type: ElementType; text: string }
...
export default function ScreenplayEditor({ initialScript, title, onTitleChange }: { initialScript: string; title: string; onTitleChange: (title: string) => void }) {
  const storedScript = localStorage.getItem('mise:screenplay') || initialScript
  const [blocks, setBlocks] = useState<Block[]>(() => scriptToBlocks(storedScript))
  ...
  const [contact, setContact] = useState(() => { const saved = localStorage.getItem('mise:contact'); return saved === 'your@email.com\n(555) 555-5555' ? '' : saved || '' })
  ...
  useEffect(() => {
    ...
    setSaveState('Saving…')
    saveTimer.current = window.setTimeout(() => { localStorage.setItem('mise:screenplay', blocksToScript(blocks)); setSaveState('Saved locally') }, 400)
  ...
  useEffect(() => {
    ...
    localStorage.setItem('mise:contact', contact)
  }, [author, contact])
  ...
  const updateBlock = (id: string, text: string) => commit((current) => current.map((block) => block.id === id ? { ...block, text: block.type === 'scene' || block.type === 'character' || block.type === 'transition' ? text.toUpperCase() : text } : block))
  const insertAt = (index: number, type: ElementType, text = '') => { const block = makeBlock(type, text); commit((current) => [...current.slice(0, index), block, ...current.slice(index)]); setActiveId(block.id); focusBlock(block.id) }
  ...
  const handleKey = (event: React.KeyboardEvent<HTMLTextAreaElement>, block: Block, index: number) => {
    ...
    if (event.key === 'Enter' && !event.shiftKey) {
      ...
      const newBlock = makeBlock(nextType(block.type), block.text.slice(event.currentTarget.selectionEnd))
      commit((current) => current.flatMap((item) => item.id === block.id ? [{ ...item, text: block.text.slice(0, start) }, newBlock] : [item]))
      setActiveId(newBlock.id); focusBlock(newBlock.id); return
    }
  ...
  const insertFromPage = (event: React.MouseEvent<HTMLElement>) => {
    ...
    insertAt(index, previousType ? nextType(previousType) : 'action')
  }
  ...
  return <div className={`editor-layout ${utilityOpen ? 'utility-open' : 'utility-closed'} ${draggingFile ? 'dragging-file' : ''}`} onDragEnter={(event) => { event.preventDefault(); setDraggingFile(true) }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDraggingFile(false) }} onDrop={(event) => { event.preventDefault(); setDraggingFile(false); const file = event.dataTransfer.files[0]; if (file) openFile(file) }}>
    ...
    <section className="desk"><div className="toolbar"><div className="format-select"><label>ELEMENT</label><select value={blocks[activeIndex]?.type || 'action'} onChange={(event) => activeId && changeType(activeId, event.target.value as ElementType)}>{Object.entries(typeLabels).map(([type, label]) => <option key={type} value={type}>{label}</option>)}</select></div><div className="tool-actions"><label className="toolbar-import">Import<input type="file" accept=".pdf,.fountain,.txt,application/pdf,text/plain" onChange={(event) => { const file = event.target.files?.[0]; if (file) openFile(file); event.currentTarget.value = '' }} /></label><button onClick={undo} disabled={historyIndex.current <= 0} title="Undo">↶</button><button onClick={redo} disabled={historyIndex.current >= history.current.length - 1} title="Redo">↷</button><button onClick={() => setFindOpen(!findOpen)}>Find</button><button onClick={createSnapshot}>Snapshot</button><button onClick={downloadFountain}>Export</button><button onClick={() => window.print()}>Print / PDF</button><button className="command-button" onClick={() => setPaletteOpen(true)}>⌘ K</button><button className="panel-toggle" onClick={() => setUtilityOpen((open) => !open)} aria-expanded={utilityOpen}>Panel</button></div><div className="save-state">{estimatedPages}p · {words} words · <b>{saveState}</b></div></div>
