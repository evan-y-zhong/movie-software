...
const readProjectStorage = (storageKey: string, name: string, migrateLegacy: boolean) => localStorage.getItem(projectStorage(storageKey, name)) ?? localStorage.getItem(projectStorage(storageKey.replace(/^logfood:/, 'mise:'), name)) ?? (migrateLegacy ? localStorage.getItem(`mise:${name}`) : null)

function loadBlocks(initialScript: string, storageKey: string, migrateLegacy: boolean) {
  try {
    const saved = JSON.parse(readProjectStorage(storageKey, 'document', migrateLegacy) || 'null')
  ...
  return scriptToBlocks(readProjectStorage(storageKey, 'screenplay', migrateLegacy) || initialScript)
}

function hideNativeDragPreview(event: React.DragEvent<HTMLElement>) {
  const preview = document.createElement('span')
  preview.style.cssText = 'position:fixed;top:-20px;left:-20px;width:1px;height:1px;opacity:0;pointer-events:none'
  document.body.appendChild(preview)
  event.dataTransfer.setDragImage(preview, 0, 0)
  requestAnimationFrame(() => preview.remove())
}

export default function ScreenplayEditor({ initialScript, title, onTitleChange, onOpenBeatMap, storageKey, migrateLegacy = false }: { initialScript: string; title: string; onTitleChange: (title: string) => void; onOpenBeatMap: () => void; storageKey: string; migrateLegacy?: boolean }) {
  const [blocks, setBlocks] = useState<Block[]>(() => loadBlocks(initialScript, storageKey, migrateLegacy))
  ...
  const [snapshots, setSnapshots] = useState<Snapshot[]>(() => JSON.parse(readProjectStorage(storageKey, 'snapshots', migrateLegacy) || '[]'))
  ...
  const [utilityOpen, setUtilityOpen] = useState(() => (localStorage.getItem('logfood:utility-panel') || localStorage.getItem('mise:utility-panel')) !== 'closed')
  const [author, setAuthor] = useState(() => { const saved = readProjectStorage(storageKey, 'author', migrateLegacy); return saved === 'YOUR NAME' ? '' : saved || '' })
  const [email, setEmail] = useState(() => { const saved = readProjectStorage(storageKey, 'email', migrateLegacy); const legacy = migrateLegacy ? localStorage.getItem('mise:contact') : null; return saved || (legacy && legacy !== 'your@email.com\n(555) 555-5555' ? legacy.split(/\r?\n/)[0] : '') || '' })
  const [phone, setPhone] = useState(() => { const saved = readProjectStorage(storageKey, 'phone', migrateLegacy); const legacy = migrateLegacy ? localStorage.getItem('mise:contact') : null; return saved || (legacy && legacy !== 'your@email.com\n(555) 555-5555' ? legacy.split(/\r?\n/).slice(1).join(' ') : '') || '' })
  ...
  const replaceWithElements = (elements: ImportedElement[]) => {
    ...
    setSnapshots((current) => { const updated = [backup, ...current]; localStorage.setItem(projectStorage(storageKey, 'snapshots'), JSON.stringify(updated)); return updated })
    blocksRef.current = next; setBlocks(next); history.current = [next]; historyIndex.current = 0; setActiveId(next[0].id)
  }
  ...
  useEffect(() => {
    ...
    setSaveState('Saving…')
    saveTimer.current = window.setTimeout(() => { localStorage.setItem(projectStorage(storageKey, 'screenplay'), blocksToScript(blocks)); localStorage.setItem(projectStorage(storageKey, 'document'), JSON.stringify(blocks)); setSaveState('Saved locally') }, 400)
    fields.current.forEach((field) => { field.style.height = 'auto'; field.style.height = `${field.scrollHeight}px` })
    return () => { if (saveTimer.current) window.clearTimeout(saveTimer.current) }
  }, [blocks])

  useEffect(() => {
    localStorage.setItem(projectStorage(storageKey, 'author'), author)
    localStorage.setItem(projectStorage(storageKey, 'email'), email)
    localStorage.setItem(projectStorage(storageKey, 'phone'), phone)
  }, [author, email, phone, storageKey])

  useEffect(() => { localStorage.setItem('logfood:utility-panel', utilityOpen ? 'open' : 'closed') }, [utilityOpen])

  useEffect(() => {
    const sceneId = localStorage.getItem(projectStorage(storageKey, 'focus-scene'))
    if (!sceneId) return
    localStorage.removeItem(projectStorage(storageKey, 'focus-scene'))
    setActiveId(sceneId)
    requestAnimationFrame(() => {
      fields.current.get(sceneId)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
      focusBlock(sceneId)
    })
  }, [storageKey])
  ...
  const moveScene = (from: number, insertionIndex: number) => {
    if (!scenes[from] || insertionIndex < 0 || insertionIndex > scenes.length) return
    ...
    const [moving] = chunks.splice(from, 1)
    const destination = insertionIndex > from ? insertionIndex - 1 : insertionIndex
    if (destination === from) return
    chunks.splice(destination, 0, moving); commit([...preamble, ...chunks.flat()])
  }
  ...
  const createSnapshot = () => { const next: Snapshot = { id: uid(), name: `Draft ${snapshots.length + 1}`, createdAt: new Date().toLocaleString(), script: blocksToScript(blocks) }; const updated = [next, ...snapshots]; setSnapshots(updated); localStorage.setItem(projectStorage(storageKey, 'snapshots'), JSON.stringify(updated)); setSnapshotsOpen(true) }
  ...
  return <div className={`editor-layout ${utilityOpen ? 'utility-open' : 'utility-closed'} ${draggingFile ? 'dragging-file' : ''}`} onDragEnter={(event) => { if (event.dataTransfer.types.includes('Files')) { event.preventDefault(); setDraggingFile(true) } }} onDragOver={(event) => { if (event.dataTransfer.types.includes('Files')) event.preventDefault() }} onDragLeave={(event) => { if (draggingFile && !event.currentTarget.contains(event.relatedTarget as Node)) setDraggingFile(false) }} onDrop={(event) => { if (!event.dataTransfer.types.includes('Files')) return; event.preventDefault(); setDraggingFile(false); const file = event.dataTransfer.files[0]; if (file) openFile(file) }}>
    <aside className="scene-nav"><div className="aside-title"><span>SCENES</span><b>{scenes.length}</b></div><nav onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setSceneDropIndex(null) }}>{scenes.map(({ block }, index) => <button key={block.id} draggable onDragStart={(event) => { event.stopPropagation(); hideNativeDragPreview(event); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('application/x-logfood-scene', String(index)); setDraggedScene(index); setSceneDropIndex(null) }} onDragEnter={(event) => event.stopPropagation()} onDragOver={(event) => { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; const bounds = event.currentTarget.getBoundingClientRect(); setSceneDropIndex(event.clientY < bounds.top + bounds.height / 2 ? index : index + 1) }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); if (draggedScene !== null && sceneDropIndex !== null) moveScene(draggedScene, sceneDropIndex); setDraggedScene(null); setSceneDropIndex(null) }} onDragEnd={() => { setDraggedScene(null); setSceneDropIndex(null) }} className={`${index === activeSceneIndex ? 'active' : ''} ${sceneDropIndex === index + 1 ? 'drop-after' : ''} ${sceneDropIndex === 0 && index === 0 ? 'drop-before' : ''}`} onClick={() => { setActiveId(block.id); fields.current.get(block.id)?.scrollIntoView({ behavior: 'smooth', block: 'center' }); focusBlock(block.id) }}><i>⠿</i><span>{String(index + 1).padStart(2, '0')}</span><p>{block.text || 'UNTITLED SCENE'}</p></button>)}</nav><button className="new-scene" onClick={() => insertAt(blocks.length, 'scene')}>＋ New scene</button></aside>
    <section className="desk"><div className="toolbar"><div className="editor-tools"><div className="format-select"><label>ELEMENT</label><select aria-label="Screenplay element" value={blocks[activeIndex]?.type || 'action'} onChange={(event) => activeId && changeType(activeId, event.target.value as ElementType)}>{Object.entries(typeLabels).map(([type, label]) => <option key={type} value={type}>{label}</option>)}</select></div><span className="toolbar-divider" /><div className="text-style-tools" aria-label="Text formatting"><button className={activeFormat.bold ? 'selected' : ''} onClick={() => toggleFormat('bold')} disabled={!activeId} aria-pressed={Boolean(activeFormat.bold)} title="Bold · ⌘B"><b>B</b></button><button className={activeFormat.italic ? 'selected' : ''} onClick={() => toggleFormat('italic')} disabled={!activeId} aria-pressed={Boolean(activeFormat.italic)} title="Italic · ⌘I"><i>I</i></button><button className={activeFormat.underline ? 'selected' : ''} onClick={() => toggleFormat('underline')} disabled={!activeId} aria-pressed={Boolean(activeFormat.underline)} title="Underline · ⌘U"><u>U</u></button><button className={activeFormat.strike ? 'selected' : ''} onClick={() => toggleFormat('strike')} disabled={!activeId} aria-pressed={Boolean(activeFormat.strike)} title="Strikethrough · ⌘⇧X"><s>S</s></button><button className={`highlight-tool ${activeFormat.highlight ? 'selected' : ''}`} onClick={() => toggleFormat('highlight')} disabled={!activeId} aria-pressed={Boolean(activeFormat.highlight)} title="Highlight line · ⌘E">H</button><button onClick={clearFormat} disabled={!activeId} title="Clear formatting">Tx</button></div><span className="toolbar-divider" /><div className="history-tools"><button onClick={undo} disabled={historyIndex.current <= 0} title="Undo">↶</button><button onClick={redo} disabled={historyIndex.current >= history.current.length - 1} title="Redo">↷</button></div></div><div className="document-tools"><label className="toolbar-import">Import<input type="file" accept=".pdf,.fountain,.txt,application/pdf,text/plain" onChange={(event) => { const file = event.target.files?.[0]; if (file) openFile(file); event.currentTarget.value = '' }} /></label><button onClick={() => setFindOpen(!findOpen)}>Find</button><button className="secondary-tool" onClick={createSnapshot}>Snapshot</button><button onClick={downloadFountain}>Export</button><button className="secondary-tool" onClick={() => window.print()}>Print / PDF</button><button className="command-button" onClick={() => setPaletteOpen(true)} title="Quick commands">⌘ K</button><button onClick={onOpenBeatMap}>Beat Map</button><button className="panel-toggle" onClick={() => setUtilityOpen((open) => !open)} aria-expanded={utilityOpen} aria-label={utilityOpen ? 'Close right panel' : 'Open right panel'}>Panel <span aria-hidden="true">{utilityOpen ? '▸' : '◂'}</span></button></div><div className="save-state"><span>{estimatedPages}p · {words} words</span><b>{saveState === 'Saving…' ? 'Saving…' : 'Saved'}</b></div></div>
